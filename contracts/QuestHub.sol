 ...
 * @dev Tracks user quest progress and completions
 */
contract QuestHub is Ownable, Pausable, ReentrancyGuard {
    ...
    event QuestCompleted(address indexed user, QuestType questType, uint256 timestamp);
    ...
    modifier onlyVaultOrBooster() {
        require(
            msg.sender == questVault || msg.sender == questBooster,
    ...
    /**
     * @notice Complete check-in quest
     */
    function completeCheckin() external whenNotPaused nonReentrant returns (uint256) {
        ...
        emit QuestCompleted(msg.sender, QuestType.CHECKIN, block.timestamp);
        return rewards.checkinReward;
    }
    
    /**
     * @notice Complete engage quest
     */
    function completeEngage() external whenNotPaused nonReentrant returns (uint256) {
        ...
        emit QuestCompleted(msg.sender, QuestType.ENGAGE, block.timestamp);
        return rewards.engageReward;
    }
    
    /**
     * @notice Complete commit quest
     */
    function completeCommit() external whenNotPaused nonReentrant returns (uint256) {
        ...
        emit QuestCompleted(msg.sender, QuestType.COMMIT, block.timestamp);
    ...
    function hasCompletedAllQuests(address _user) external view returns (bool) {
        ...
        uint256 sessionWindow = 1 hours; // All quests must be within 1 hour
